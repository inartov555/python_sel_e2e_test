...
7. Run the `run_tests.sh` file next way: `source run_tests.sh $PATH_TO_INI_CONFIG` (if you don't pass ini config file, then default one is used [`pytest.ini`](./pytest.ini)). 
...
- **Selenium** chosen just to show ability to work with this framework
